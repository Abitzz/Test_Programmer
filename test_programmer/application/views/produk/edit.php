...
<body>
    <div class="form-container">
        ...
        <form action="<?= site_url('produk/update/'.$produk->id_produk) ?>" method="post" onsubmit="return validateForm()" class="form-table">
            <div class="form-row">
                <label for="nama_produk">Nama Produk:</label>
                <input type="text" id="nama_produk" name="nama_produk" value="<?= set_value('nama_produk', $produk->nama_produk); ?>">
                <?= form_error('nama_produk', '<small class="error">', '</small>'); ?>
            </div>

            <div class="form-row">
                <label for="harga">Harga:</label>
                <input type="text" id="harga" name="harga" value="<?= set_value('harga', $produk->harga); ?>">
            ...
            <div class="form-row">
                ...
                <select name="status" id="status" class="select-status">
                    <option value="1" <?= $produk->id_status == 1 ? 'selected' : ''; ?>>Bisa Dijual</option>
                    <option value="2" <?= $produk->id_status == 2 ? 'selected' : ''; ?>>Tidak Bisa Dijual</option>
                </select>
            </div>
